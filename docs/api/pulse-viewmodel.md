...
Cancels the work started in `onSetup()` for good, without replacing the scope. `onCleared()` calls it when the owning `ViewModelStore` is cleared, so a discarded ViewModel cannot launch anything that outlives it.
