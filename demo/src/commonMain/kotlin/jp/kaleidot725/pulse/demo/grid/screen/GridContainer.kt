 ...
 *
 * A tapped area counts the tap itself and reports it upwards; the Container sends it back down to
 * everyone. The broadcast goes to all four rather than to a computed set of neighbours: the
...
class GridContainer(
    ...
    override fun onReceived(unicast: GridUnicast) {
        when (unicast) {
            is GridUnicast.Pulsed -> relayPulse(unicast.origin)
        }
    }

    private fun relayPulse(origin: AreaPosition) {
        broadcast(GridBroadcast.Pulse(origin))
    }
}
